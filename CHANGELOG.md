...
- `docs/guide/time.md` - virtual time guide
  ([#13](https://github.com/galeon-engine/galeon/issues/13))
- Hot-reload for RON data files: `AssetEvent`, `AssetEvents`, `FileWatcher`
  (behind `hot-reload` feature flag), `DataRegistry::reload_unit()`,
  `HotReloadPlugin`, and `TemplateRef` component for automatic entity patching
  ([#10](https://github.com/galeon-engine/galeon/issues/10))
- `UnitStats` now implements `Component` for ECS storage
  ([#10](https://github.com/galeon-engine/galeon/issues/10))
- `docs/guide/hot-reload.md` documenting feature flag setup, architecture,
  and limitations
  ([#10](https://github.com/galeon-engine/galeon/issues/10))
