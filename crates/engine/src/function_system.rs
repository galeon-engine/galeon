...
// Type alias for clarity in legacy system
    ...
    ($($P:ident),+) => {
            ...
            Func: FnMut($($P::Item<'_>),+) + 'static,
        {
            fn run(&mut self, world: &mut World) {
                ...
                // SAFETY: access conflicts checked at registration.
                unsafe {
        ...
        where Func: SystemParamFunction<($($P,)+)> {
            fn into_system(self, name: &'static str) -> Box<dyn System> {
...
mod tests {
    ...
    fn query_mut_system_mutates() {
        ...
        sys.run(&mut world);
        let vals: Vec<u32> = world.query::<Counter>().iter().map(|(_, c)| c.0).collect();
