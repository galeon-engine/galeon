...
/// Extract `Ok` and `Err` type syntax from a `Result<R, E>` return type.
fn extract_result_pair(ty: &syn::Type) -> Result<(syn::Type, syn::Type), syn::Error> {
        ...
        if type_args.len() == 2 {
            let ok_type = match type_args[0] {
                ...
                _ => {
                    return Err(syn::Error::new_spanned(
                        type_args[0],
                        "expected type parameter for Result Ok type",
                    ));
                }
            };
            let err_type = match type_args[1] {
                ...
                _ => {
                    return Err(syn::Error::new_spanned(
                        type_args[1],
                        "expected type parameter for Result Err type",
                    ));
                }
            };
...
fn handler_attr(_attr: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let (response_type, err_type) = match extract_result_pair(return_type) {
        Ok(types) => types,
        Err(e) => return e.to_compile_error().into(),
    };
    let response_type_str = type_to_string(&response_type);
    let error_type_str = type_to_string(&err_type);
    ...
    let json_shim_ident = format_ident!("{}__galeon_axum_json", fn_ident);

    let expanded = quote! {
        #func

        /// Autogenerated by `#[handler]` for `galeon generate routes` axum glue.
        #[doc(hidden)]
        #[allow(non_snake_case)]
        pub fn #json_shim_ident(
            json: &str,
            world: &mut #krate::World,
        ) -> Result<#krate::serde_json::Value, String> {
            let mut h = #krate::handler_function::IntoHandler::<
                #request_type,
                #response_type,
                #params_tuple,
            >::into_handler(#fn_ident, #name_str);
            #krate::run_json_handler_value(&mut *h, json, world)
        }
